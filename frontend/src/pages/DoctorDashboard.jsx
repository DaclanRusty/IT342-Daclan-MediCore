...
// ── Helpers: normalize field names from backend ───────────────────────────
// Backend secretary requests return firstname/lastname (from AuthResponse)
const secFn   = s => s?.firstName || s?.firstname || '';
const secLn   = s => s?.lastName  || s?.lastname  || '';
const secEmail = s => s?.email    || '';

// Backend appointments return camelCase (from AppointmentResponse)
...
const Navbar = ({ user, onLogout }) => {
  // ← FIX: normalize both camelCase and lowercase from auth context
    ...
    <nav style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span style={{ fontWeight: 700, fontSize: 16, color: '#1e293b' }}>
          Medi<span style={{ color: '#10b981' }}>Core</span>
        </span>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{
          display: 'flex', alignItems: 'center', gap: 9,
          background: '#f0fdfb', border: '1px solid #ccfbf1',
  ...
  <div style={{
    ...
    {TABS.map(t => (
      <button key={t.key} onClick={() => onChange(t.key)} style={{
        ...
        borderBottom: active === t.key ? '2px solid #10b981' : '2px solid transparent',
        color: active === t.key ? '#10b981' : '#64748b',
...
const DashboardTab = ({ appointments, secretaryRequests, onGoTo, user }) => {
  ...
  // ← FIX: normalize date field — backend returns requestedDate (camelCase)
  ...
  // ← FIX: normalize secretary name fields
  ...
  return (
    <div style={{ padding: '32px', maxWidth: 1000, margin: '0 auto' }}>
      ...
      <p style={{ color: '#94a3b8', fontSize: 13, marginBottom: 28 }}>
        Manage your appointments and secretary
      </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 16, marginBottom: 36 }}>
        {[
          ...
          <div key={label} style={{
            ...
            border: `1.5px solid ${border}`, boxShadow: '0 2px 8px rgba(0,0,0,0.05)',
          }}>
            <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          {pendingSec > 0 && (
            <button onClick={() => onGoTo('secretary')} style={{
        ...
        {assignedSec ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              {/* ← FIX: use normalizer helpers */}
              <div style={{ fontWeight: 700, fontSize: 15, color: '#0f172a' }}>
                {secFn(assignedSec)} {secLn(assignedSec)}
              </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: '#94a3b8' }}>
                Secretaries can request to be assigned during registration
              </div>
            </div>
          </div>
        )}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <button onClick={() => onGoTo('appointments')} style={{
        ...
        {appointments.length === 0 ? (
          ...
          appointments.slice(0, 4).map(a => (
            <div key={a.id} style={{
              ...
              padding: '14px 20px', borderBottom: '1px solid #f8fafc', gap: 12,
            }}>
              <div style={{
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: '#94a3b8', display: 'flex', gap: 8, marginTop: 2 }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                    <CalendarIcon /> {apptDate(a)}
                  </span>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                    <ClockIcon /> {apptTime(a)}
                  </span>
                </div>
              </div>
  ...
  return (
    <div style={{ padding: '32px', maxWidth: 1000, margin: '0 auto' }}>
      ...
      <p style={{ color: '#64748b', fontSize: 13, marginBottom: 20 }}>
        All approved appointments assigned to you ({appointments.length} total)
      </p>
...
const SecretaryRequestsTab = ({ requests, onApprove, onReject, loading }) => {
  ...
  return (
    <div style={{ padding: '32px', maxWidth: 1000, margin: '0 auto' }}>
      ...
      <p style={{ color: '#64748b', fontSize: 13, marginBottom: 20 }}>
        Secretaries who selected you during registration
      </p>
      {hasApproved && (
        <div style={{
          ...
          <span style={{ fontSize: 13, color: '#059669', fontWeight: 600 }}>
            You already have an assigned secretary.
          </span>
        </div>
      )}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {requests.length === 0 ? (
          <div style={{
            ...
            boxShadow: '0 2px 12px rgba(0,0,0,0.06)',
          }}>No secretary requests yet</div>
        ) : (
          requests.map(req => (
            <div key={req.secretaryId} style={{
              background: '#fff', borderRadius: 16, padding: 20,
              boxShadow: '0 2px 12px rgba(0,0,0,0.06)',
              ...
              <div style={{ flex: 1 }}>
                ...
                {req.requestedAt && (
                  <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>
                    Requested: {new Date(req.requestedAt).toLocaleDateString('en-PH', {
              ...
              {req.status === 'PENDING' && !hasApproved && (
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => onApprove(req.secretaryId)} disabled={loading} style={{
                  ...
                  <button onClick={() => onReject(req.secretaryId)} disabled={loading} style={{
                    padding: '8px 16px', borderRadius: 8, border: 'none',
...
const ProfileTab = ({ onSaved }) => {
  ...
  const syncPhone = (phoneNumber) => {
    if (phoneRef.current) {
      phoneRef.current.value = phoneNumber
        ? formatPHPhone(phoneNumber).display
        : '+63 ';
    }
  };
  ...
  // ← FIX: fetch profile on mount — this was broken by previous edits
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        ...
        setSaveErr('Failed to load profile: ' + e.message);
      } finally {
        setLoading(false);
      }
    })();
  ...
  useEffect(() => {
    if (editing) syncPhone(form.phoneNumber);
  }, [editing]);
  ...
  const handleSave = async () => {
    ...
    if (isChangingPassword) {
      if (!form.currentPassword) {
        setSaveErr('Enter your current password to change it.');
        return;
      }
      if (form.newPassword.length < 8) {
        setSaveErr('New password must be at least 8 characters.');
        return;
      }
      if (form.newPassword !== form.confirmPassword) {
        setSaveErr('New passwords do not match.');
        return;
      }
    }
    setSaving(true);
  ...
  const FieldRow = ({ label, value, last }) => (
    <div style={{
      ...
      borderBottom: last ? 'none' : '1px solid rgba(226,232,240,.5)',
    }}>
      <div style={{ minWidth: 180, fontSize: 12.5, fontWeight: 600, color: '#94a3b8', paddingTop: 1 }}>
        {label}
      </div>
      <div style={{
        fontSize: 14, color: value ? '#0f172a' : '#cbd5e1',
        fontStyle: value ? 'normal' : 'italic', flex: 1,
      }}>{value || 'Not provided'}</div>
    </div>
  );
        ...
        <div style={{ background: 'linear-gradient(135deg,#10b981,#059669)', padding: '28px 32px 24px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 20, flexWrap: 'wrap' }}>
            <div style={{
              ...
              background: 'rgba(255,255,255,.2)', border: '3px solid rgba(255,255,255,.55)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              color: '#fff', fontWeight: 900, fontSize: 28, flexShrink: 0,
            }}>{initials}</div>
            <div style={{ flex: 1, minWidth: 160 }}>
              <div style={{ color: 'rgba(255,255,255,.7)', fontSize: 11.5, fontWeight: 700, letterSpacing: .7, marginBottom: 3 }}>
                DOCTOR PROFILE
              </div>
              ...
              {profile?.bio && (
                <div style={{ color: 'rgba(255,255,255,.7)', fontSize: 12.5, marginTop: 6, fontStyle: 'italic', maxWidth: 460 }}>
                  "{profile.bio}"
                </div>
              )}
            </div>
            {!editing ? (
              <button onClick={() => { setSaveErr(''); setEditing(true); }} style={{
                display: 'flex', alignItems: 'center', gap: 7, padding: '9px 18px',
                borderRadius: 10, border: '1.5px solid rgba(255,255,255,.5)',
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={handleSave} disabled={saving} style={{
                  display: 'flex', alignItems: 'center', gap: 7, padding: '9px 18px',
                  borderRadius: 10, border: 'none', background: '#fff',
        ...
        <div style={{ padding: '28px 32px' }}>
          {saveErr && (
            <div style={{
              ...
              padding: '11px 16px', color: '#991b1b', fontWeight: 600,
          ...
          {!editing && (
            <>
              <div style={{ marginBottom: 24 }}>
                <div style={{ fontSize: 11.5, fontWeight: 700, color: '#10b981', letterSpacing: .8, marginBottom: 12 }}>
          ...
          {editing && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {/* Personal */}
              <div style={{ marginBottom: 24 }}>
                <div style={{ fontSize: 11.5, fontWeight: 700, color: '#10b981', letterSpacing: .8, marginBottom: 16 }}>
                  👤 PERSONAL INFORMATION
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
                  <EditField label="First Name">
                    <input style={inp} value={form.firstName} onChange={set('firstName')} placeholder="First name" />
                  </EditField>
                  <EditField label="Last Name">
                    <input style={inp} value={form.lastName} onChange={set('lastName')} placeholder="Last name" />
                  </EditField>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  <EditField label="Phone Number" hint="Format: +63 9xx xxx xxxx">
                    <input
                      ...
                      type="tel"
                      style={inp}
                      defaultValue={form.phoneNumber ? formatPHPhone(form.phoneNumber).display : '+63 '}
                      onChange={handlePhoneChange}
                      onFocus={e => { if (!e.target.value) e.target.value = '+63 '; }}
              ...
              <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: 20, marginBottom: 24 }}>
                ...
                <EditField label="Short Bio / Description" hint="Optional — briefly describe your practice">
              ...
              <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: 20 }}>
                <div style={{ fontSize: 11.5, fontWeight: 700, color: '#10b981', letterSpacing: .8, marginBottom: 4 }}>
                  🔒 CHANGE PASSWORD
                </div>
    ...
    <div style={{
      ...
      {tab === 'dashboard' && (
        <DashboardTab
          ...
          secretaryRequests={secretaryRequests}
          onGoTo={setTab}
          user={user}
        />
      )}
      ...
      {tab === 'secretary' && (
        <SecretaryRequestsTab
          requests={secretaryRequests}
          onApprove={handleApproveSecretary}
          onReject={handleRejectSecretary}
