  ...
  <>
    ...
    <style>{`
      *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
      ...
      .doc-card{background:rgba(255,255,255,.85);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border:1.5px solid rgba(255,255,255,.95);border-radius:18px;padding:18px 20px;box-shadow:0 2px 16px rgba(37,99,235,.05);transition:transform .22s,box-shadow .22s;display:flex;flex-direction:column;}
...
// ── B&W inline SVG icons for appointments list ────────────────────────────
const DateIcon = () => (
...
// ── Stat card icons (white stroke, renders on gradient bg) ────────────────
...
const HomeTab = ({user,appts,apptLoad,apptErr,onBook,onGoTo,onRetryAppts,tips,tipsLoad,tipsErr,onRetryTips}) => {
  ...
  // ── Stat cards: SVG icons, no emojis, number centered ──
  const statCards = [
    { label:"Total Appointments",     val:counts.total,     grad:`linear-gradient(135deg,${C.blue},${C.blueDk})`,  icon:<StatClipboardIcon/> },
    { label:"Approved Appointments",  val:counts.approved,  grad:`linear-gradient(135deg,${C.green},#047857)`,      icon:<StatCheckIcon/>     },
  ...
  return (
    <div className="pw" style={{padding:"0 32px 56px",maxWidth:1120,margin:"0 auto"}}>
      ...
      {/* ── Stat cards: centered number, no emoji ── */}
            ...
            {upcoming.map(a=>{
              ...
              return (
                <div key={a.id} className="appt-card" style={{padding:"18px 20px"}}>
                  ...
                  {/* B&W SVG date/time icons replacing emojis */}
            ...
            return (
              <div key={a.id} className="appt-card au3" style={{padding:"18px 22px",display:"flex",alignItems:"center",gap:16,flexWrap:"wrap",animationDelay:`${i*.05}s`}}>
                ...
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8}}>
                  ...
                  {st==="APPROVED"&&<span style={{fontSize:11.5,color:C.green,fontWeight:600,background:C.greenLt,border:`1px solid ${C.greenBdr}`,padding:"4px 10px",borderRadius:8}}>✓ Appointment confirmed</span>}
                  {st==="REJECTED"&&<span style={{fontSize:11.5,color:"#991b1b",fontWeight:600,background:C.redLt,border:`1px solid ${C.redBdr}`,padding:"4px 10px",borderRadius:8}}>✗ Not approved</span>}
                </div>
              </div>
            );
          })}
...
// ─────────────────────────────────────────────────────────────────────────────
// DOCTORS TAB — bio + years of experience only (no license number)
// ─────────────────────────────────────────────────────────────────────────────
              ...
              <div key={id} className="doc-card" style={{animationDelay:`${i*.05}s`}}>
                ...
                <div style={{display:"flex",alignItems:"flex-start",gap:13,marginBottom:12}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:12.5,color:C.slateL,marginTop:2}}>
                      {doc.specialization||"General Practitioner"}
                    </div>
                    {/* Years of experience — replaces license number */}
                    {yoe && (
                ...
                {/* Bio — replaces phone/license */}
                {bio ? (
                  <p style={{fontSize:12.5,color:C.slateM,lineHeight:1.65,marginBottom:14,display:"-webkit-box",WebkitLineClamp:3,WebkitBoxOrient:"vertical",overflow:"hidden",flexGrow:1}}>
                    {bio}
                  </p>
                ) : (
                ...
                <button className="btn-primary" onClick={()=>onBook(doc)} style={{width:"100%",justifyContent:"center",padding:"10px",fontSize:13,marginTop:"auto"}}>
                  Book Appointment
                </button>
              </div>
            );
...
const ProfileTab = ({ user }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveErr(e.message || "Failed to save changes. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:80,gap:10,color:C.slateL,fontSize:14,fontWeight:600}}>
  ...
  if (fetchErr) return (
    <div style={{padding:"32px",maxWidth:760,margin:"0 auto"}}>
      <div style={{background:C.redLt,border:`1.5px solid ${C.redBdr}`,borderRadius:14,padding:"16px 20px",color:"#991b1b",fontWeight:600,fontSize:13.5,display:"flex",alignItems:"center",gap:10}}>
        ⚠️ {fetchErr}
        <button onClick={()=>window.location.reload()} style={{marginLeft:12,background:"none",border:"none",color:C.blue,fontWeight:700,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontSize:13}}>Retry</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="pw" style={{padding:"24px 32px 56px",maxWidth:760,margin:"0 auto"}}>
      {saveOk && (
        <div className="au1" style={{background:C.greenLt,border:`1.5px solid ${C.greenBdr}`,borderRadius:14,padding:"12px 18px",display:"flex",alignItems:"center",gap:10,marginBottom:16,color:"#166534",fontWeight:700,fontSize:13.5}}>
          ✅ Profile updated successfully!
        </div>
      )}
      <div className="glass au2" style={{padding:"28px 32px",marginBottom:18}}>
        <div style={{display:"flex",alignItems:"center",gap:20,marginBottom:24,flexWrap:"wrap"}}>
          <div style={{width:78,height:78,borderRadius:"50%",background:`linear-gradient(135deg,${C.blue},${C.purple})`,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontFamily:"'Sora',sans-serif",fontWeight:900,fontSize:26,boxShadow:`0 6px 20px rgba(37,99,235,.28)`,flexShrink:0}}>
            {getInitials(fn,ln)||"P"}
          </div>
          <div style={{flex:1,minWidth:160}}>
            ...
            <div style={{display:"flex",gap:8,marginTop:8}}>
              ...
              <span style={{background:profile?.status==="ACTIVE"?C.greenLt:"#f1f5f9",color:profile?.status==="ACTIVE"?C.green:C.slateL,border:`1px solid ${profile?.status==="ACTIVE"?C.greenBdr:"#cbd5e1"}`,borderRadius:100,padding:"3px 11px",fontSize:11.5,fontWeight:700}}>
        ...
        {saveErr && (
          <div style={{background:C.redLt,border:`1.5px solid ${C.redBdr}`,borderRadius:12,padding:"11px 16px",color:"#991b1b",fontWeight:600,fontSize:13,marginBottom:18,display:"flex",alignItems:"center",gap:8}}>
            ⚠️ {saveErr}
          </div>
        )}
        {!editing && (
          ...
          <div style={{display:"flex",flexDirection:"column",gap:0}}>
            <div style={{borderTop:"1.5px solid rgba(226,232,240,.6)",paddingTop:20,marginBottom:24}}>
                  ...
                  <input
                    ...
                    onChange={e => {
                      ...
                      if (digits.startsWith("63")) {
                        ...
                        set("phoneNumber")({ target: { value: formatted } });
                      } else {
                        set("phoneNumber")({ target: { value: digits ? "+" + digits : "" } });
                      }
                    }}
              ...
              <div>
                ...
                <div style={{padding:"10px 14px",borderRadius:11,border:"1.5px solid rgba(226,232,240,.6)",background:"rgba(241,245,249,.6)",fontSize:14,color:C.slateXL,fontStyle:"italic"}}>
                  {profile?.email} <span style={{fontSize:11.5}}>(cannot be changed)</span>
                </div>
              </div>
            </div>
            <div style={{borderTop:"1.5px solid rgba(226,232,240,.6)",paddingTop:20,marginBottom:24}}>
              ...
              <EditField label="Address">
                <textarea className="input-field" rows={3} value={form.address} onChange={set("address")} placeholder="Street, City, Province"/>
              </EditField>
            </div>
                  ...
                  <EditField key={key} label={label} hint={hint}>
                    <div style={{position:"relative"}}>
                      ...
                      <button type="button" onClick={()=>setShowPw(p=>({...p,[vis]:!p[vis]}))}
                        style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:C.slateXL,display:"flex",alignItems:"center"}}>
